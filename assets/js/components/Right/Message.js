...
class Message extends React.Component {
    ...
    {
        let img = ``;
        let mine = this.props.message.recipientUser.username === store.getState().username;
        ...
        return (
            <div className={`media w-50 mb-3 ${!mine ? `ml-auto` : ``}`} >
                ...
                <div className="media-body ml-3">
                    <div className={`rounded py-2 px-3 mb-2 ${!mine ? 'bg-primary' : 'bg-light'}`}>
                        <p className={`text-small mb-0 ${!mine ? 'text-white' : 'text-muted'}`}>{this.props.message.content}</p>
                    </div>
